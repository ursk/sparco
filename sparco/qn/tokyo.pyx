...
cdef void sswap(np.ndarray x, np.ndarray y):
    ...
    if y.descr.type_num != np.NPY_FLOAT:
...
cdef void dswap(np.ndarray x, np.ndarray y):
    ...
    if y.ndim != 1: raise ValueError("y is not a vector")
    if x.shape[0] != y.shape[0]: raise ValueError("x rows != y rows")
    if x.descr.type_num != np.NPY_DOUBLE:
        raise ValueError("x is not of type double")
    if y.descr.type_num != np.NPY_DOUBLE:
...
# hacking out parts that require atlas
...
cdef tuple srotmg(float d1, float d2, float x, float y, np.ndarray param):
    ...
    if param.descr.type_num != np.NPY_FLOAT:
...
cdef tuple drotmg(double d1, double d2, double x, double y, np.ndarray param):
    ...
    if param.shape[0] < 5:
        raise ValueError("param must have length at least 5")
    if param.descr.type_num != np.NPY_DOUBLE:
...
cdef void sgemv6(CBLAS_TRANSPOSE TransA, float alpha, np.ndarray A,
    ...
    if A.descr.type_num != np.NPY_FLOAT: raise ValueError("A is not of type float")
    if x.descr.type_num != np.NPY_FLOAT: raise ValueError("x is not of type float")
    if y.descr.type_num != np.NPY_FLOAT: raise ValueError("y is not of type float")
...
cdef void dgemv6(CBLAS_TRANSPOSE TransA, double alpha, np.ndarray A,
    ...
    if A.shape[0] != y.shape[0]: raise ValueError("A rows != y rows")
    if A.shape[1] != x.shape[0]: raise ValueError("A columns != x rows")
    if A.descr.type_num != np.NPY_DOUBLE: raise ValueError("A is not of type double")
    if x.descr.type_num != np.NPY_DOUBLE: raise ValueError("x is not of type double")
    if y.descr.type_num != np.NPY_DOUBLE: raise ValueError("y is not of type double")
...
cdef void sgemm7(CBLAS_TRANSPOSE TransA, CBLAS_TRANSPOSE TransB,
    ...
    if A.descr.type_num != np.NPY_FLOAT: raise ValueError("A is not of type float")
    if B.descr.type_num != np.NPY_FLOAT: raise ValueError("B is not of type float")
    if C.descr.type_num != np.NPY_FLOAT: raise ValueError("C is not of type float")
...
cdef void dgemm7(CBLAS_TRANSPOSE TransA, CBLAS_TRANSPOSE TransB,
    ...
    if A.shape[1] != B.shape[0]: raise ValueError("A columns != B rows")
    if A.descr.type_num != np.NPY_DOUBLE: raise ValueError("A is not of type double")
    if B.descr.type_num != np.NPY_DOUBLE: raise ValueError("B is not of type double")
    if C.descr.type_num != np.NPY_DOUBLE: raise ValueError("C is not of type double")
...
cdef np.ndarray smnewempty(int M, int N):
    ...
    ref.Py_INCREF(np.NPY_FLOAT) # This is apparently necessary
    return np.PyArray_EMPTY(2, length, np.NPY_FLOAT, 0)
...
cdef np.ndarray dmnewempty(int M, int N):
    cdef np.npy_intp length[2]
    length[0] = M; length[1] = N
    ref.Py_INCREF(np.NPY_DOUBLE) # This is apparently necessary
    return np.PyArray_EMPTY(2, length, np.NPY_DOUBLE, 0)
...
cdef np.ndarray svnewempty(int M):
    ...
    length[0] = M
    ref.Py_INCREF(np.NPY_FLOAT) # This is apparently necessary
    return np.PyArray_EMPTY(1, length, np.NPY_FLOAT, 0)
...
cdef np.ndarray dvnewempty(int M):
    ...
    length[0] = M
    ref.Py_INCREF(np.NPY_DOUBLE) # This is apparently necessary
    return np.PyArray_EMPTY(1, length, np.NPY_DOUBLE, 0)
...
cdef np.ndarray smnewzero(int M, int N):
    ...
    length[0] = M; length[1] = N
    ref.Py_INCREF(np.NPY_FLOAT) # This is apparently necessary
    return np.PyArray_ZEROS(2, length, np.NPY_FLOAT, 0)
...
cdef np.ndarray dmnewzero(int M, int N):
    ...
    length[0] = M; length[1] = N
    ref.Py_INCREF(np.NPY_DOUBLE) # This is apparently necessary
    return np.PyArray_ZEROS(2, length, np.NPY_DOUBLE, 0)
...
cdef np.ndarray svnewzero(int M):
    ...
    length[0] = M
    ref.Py_INCREF(np.NPY_FLOAT) # This is apparently necessary
    return np.PyArray_ZEROS(1, length, np.NPY_FLOAT, 0)
...
cdef np.ndarray dvnewzero(int M):
    ...
    length[0] = M
    ref.Py_INCREF(np.NPY_DOUBLE) # This is apparently necessary
    return np.PyArray_ZEROS(1, length, np.NPY_DOUBLE, 0)
